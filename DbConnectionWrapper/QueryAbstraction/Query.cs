...
public class Query
{
    ...
    /// </summary>
    /// <remarks>
    ///     This callback passes DbConnection and DbTransaction to allow for further synchronous calls after the current query
    ///     runs.
    ///     For example: you run a query to check if a table exists, and then use a callback for it to execute a query to
    ///     create the table if it doesnt exist.
    ///     Note that throwing an exception during a callback when the callback was from a ExecuteTransaction, will result in
    ///     the transaction rolling back and not commit any of the data or changes on the transaction.
    ...
    /// </summary>
    /// <remarks>
    ///     This callback passes DbConnection and DbTransaction to allow for further asynchronous calls after the current query
    ///     runs.
    ///     For example: you run a query to check if a table exists, and then use a callback for it to execute a query to
    ///     create the table if it doesnt exist.
    ///     Note that throwing an exception during a callback when the callback was from a ExecuteTransaction, will result in
